...
project=root/'output/Namaa-hero-refined'
archive=root/'output/Namaa-hero-refined.zip'
assert project.resolve().is_relative_to(root.resolve())
project.mkdir(parents=True,exist_ok=True)
for name in ['index.html','README.md','DELIVERY.md','favicon.ico','apple-touch-icon.png','assets','css','js']:
    source,target=root/name,project/name
    if source.is_dir():shutil.copytree(source,target,dirs_exist_ok=True)
    else:shutil.copy2(source,target)
for screenshot in review.glob('*.png'):
    with Image.open(screenshot) as image:
        if image.format!='PNG':image.convert('RGB').save(screenshot,'PNG',optimize=True)
notes='''# Hero refinement only
...
'''
(review/'HERO-REVIEW.md').write_text(notes,encoding='utf-8')
with zipfile.ZipFile(archive,'w',zipfile.ZIP_DEFLATED,compresslevel=9) as bundle:
    for p in sorted(project.rglob('*')):
        if p.is_file():bundle.write(p,'Namaa-hero-refined/'+p.relative_to(project).as_posix())
    for name in ['HERO-REVIEW.md','hero-desktop-1440.png','hero-mobile-390.png','responsive-qa.json','scope-check.json','crop-source.json']:
        bundle.write(review/name,'Review/'+name)
with zipfile.ZipFile(archive) as bundle:
    assert bundle.testzip() is None
    for p in project.rglob('*'):
        if p.is_file():assert bundle.read('Namaa-hero-refined/'+p.relative_to(project).as_posix())==p.read_bytes()
print('ZIP integrity and copied project verified:',archive.stat().st_size,'bytes')
