...
(function () {
  ...
  /* Approved Logix products; source links and images stay in the data file. */
  var hardwareGrid = document.getElementById('hardware-products');
  var hardwareList = document.getElementById('hardware-options-list');
  var hardwareOptions = document.getElementById('hardware-options');
  var hardwareToggle = document.getElementById('hardware-options-button');
  function sourceProductUrl(value) {
    try {
      var url = new URL(value);
      return url.protocol === 'https:' && url.hostname === 'logix-mobile.com' && !url.username && !url.password ? url.href : '';
    } catch (_) { return ''; }
  }
  function hardwareCard(product, compact) {
    var article = document.createElement('article');
    article.className = compact ? 'hardware-option' : 'hardware-card';
    ...
    var title = document.createElement(compact ? 'h4' : 'h3');
    title.textContent = (!compact && product.featuredName) || product.name;
    var description = document.createElement('p');
    description.textContent = (!compact && product.featuredDescription) || product.description;
    var price = document.createElement('p');
    price.className = 'hardware-price';
    if (product.approvedPrice === true && typeof product.price === 'number' && Number.isFinite(product.price) && product.price >= 0) {
      if (!compact && product.featuredPricePrefix) price.appendChild(document.createTextNode(product.featuredPricePrefix));
      var amount = document.createElement('bdi');
      amount.textContent = product.price + ' ' + product.currency;
      price.appendChild(amount);
    } else {
      price.textContent = 'استفسر عن السعر';
    }
    var source = document.createElement('a');
    source.className = 'hardware-product-link';
    source.href = sourceProductUrl(product.sourceUrl);
    source.target = '_blank';
    source.rel = 'noopener noreferrer';
    source.textContent = 'التفاصيل لدى Logix';
  ...
  var approvedHardware = (window.HARDWARE_PRODUCTS || []).filter(function (product) {
    return product.approved === true && sourceProductUrl(product.sourceUrl) && /^assets\/hardware\/[a-z0-9-]+\.webp$/.test(product.image) && product.width > 0 && product.height > 0;
  });
  if (hardwareGrid && hardwareList && hardwareOptions && hardwareToggle && approvedHardware.length) {
    approvedHardware.filter(function (product) { return product.featured; }).slice(0, 4).forEach(function (product) {
      hardwareGrid.appendChild(hardwareCard(product, false));
    });
    var hardwareLoaded = false;
    function setHardwareOptions(open, focusToggle) {
      hardwareOptions.hidden = !open;
      hardwareToggle.setAttribute('aria-expanded', String(open));
      hardwareToggle.textContent = open ? 'إخفاء خيارات الأجهزة' : 'عرض خيارات الأجهزة';
      if (focusToggle) hardwareToggle.focus({ preventScroll: true });
    }
    hardwareToggle.hidden = false;
    hardwareToggle.addEventListener('click', function () {
      if (!hardwareLoaded) {
        approvedHardware.forEach(function (product) { hardwareList.appendChild(hardwareCard(product, true)); });
        hardwareLoaded = true;
      }
      setHardwareOptions(hardwareOptions.hidden, false);
    });
    document.getElementById('hardware').addEventListener('keydown', function (event) {
      if (event.key === 'Escape' && !hardwareOptions.hidden) {
        event.preventDefault();
        setHardwareOptions(false, true);
      }
    });
  }

  /* No section or placeholder video when the approved tutorials list is empty. */
  ...
  var tutorialsTemplate = document.getElementById('tutorials-template');
  var faqSection = document.getElementById('faq');
  if (tutorials.length && tutorialsTemplate && faqSection) {
    var tutorialSection = tutorialsTemplate.content.firstElementChild.cloneNode(true);
    tutorials.forEach(function (tutorial) {
      ...
      watch.setAttribute('aria-label', 'شاهد شرح ' + tutorial.title);
      card.append(title, description, watch);
      tutorialSection.querySelector('.tutorials-grid').appendChild(card);
    });
    faqSection.before(tutorialSection);
  }
